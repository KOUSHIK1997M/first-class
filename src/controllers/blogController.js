...
const deleteByblogId = async function (req, res) {
    try {
        let Id = req.Id;
        let data = req.params.blogId;
        let blogId = await blogModel.findById(ObjectId(data))
        if (!blogId == false) {
            if (blogId.authorId == Id) {
                let deleted = await blogModel.findById(ObjectId(data)).find({ isDeleted: true });
                if (deleted.length == 1) res.status(200).send({ status: true, msg: "Data already deleted." })
                else {
                    let data1 = blogId._id
                    let time = moment().format(process.env.TIME);
        ...
        } else res.status(404).send({ status: false, msg: "Invalide author Id" })
    } catch (error) {
        res.status(500).send({ status: false, msg: error.message })
    }
}
        ...
        if (newData.length < 1) {
            ...
            if (deleted.length == 1) {
                ...
                if (newData[0].authorId == Id) {
                    ...
                    res.status(403).send({ status: false, msg: "invalid authorization persona" }) //If don't have any data in newData then send message.
                }
            }
        }
    } catch (error) {
